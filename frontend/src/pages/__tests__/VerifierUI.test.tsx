...
vi.mock('../../lib/contracts/credentialProtocol', () => ({
  getCredential: vi.fn(),
...
describe('VerifierUI', () => {
  ...
  it('fetches and displays credential details on valid ID', async () => {
    const { getCredential, getAttestors, isExpired, isAttested } = await import('../../lib/contracts/credentialProtocol');
  ...
  it('shows credential list when address has credentials', async () => {
    const { getCredentialsBySubject } = await import('../../lib/contracts/credentialProtocol');
